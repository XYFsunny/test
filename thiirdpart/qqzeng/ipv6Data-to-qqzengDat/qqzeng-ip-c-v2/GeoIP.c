...
#include <error.h>
#include <time.h>
char *IP_FILENAME = "output-geoout02.dat";
//char *IP_FILENAME = "output-100w.dat";
//char *IP_FILENAME = "output-200w.dat";
//char *IP_FILENAME = "output-all.dat";
...
int32_t geoip_loadDat(geo_ip *p)
{
    ...
    for (i = 0; i < RecordSize; i++)
	{
		j = 2052 + (i * 10);
		p->endArr[i] = geoip_read_int32(p, buffer, j);
		offset = geoip_read_int32(p, buffer, 4 + j);
		length = geoip_read_int16(p, buffer, 8 + j);
		char *result = (char *)malloc((length + 1) * sizeof(char));
		memcpy(result, buffer + offset, length);
		result[length] = '\0';
		p->addrArr[i] = result;
...
uint16_t geoip_read_int16(geo_ip *p, uint8_t *buf, int pos)
{
	uint32_t result;
	result = (uint16_t)( (buf[pos + 1] << 8 & 0xff00) | (buf[pos] & 0xff));
	return result;
}
//##define num 10000000
#define num 10
...
void longToIp(char *p,uint32_t adr) {
        sprintf(p, "%d.%d.%d.%d", adr >> 24, (adr >> 16) & 0xff, (adr >> 8) & 0xff, adr & 0xff);

}

void test(geo_ip *finder)
{
    ...
    for( uint32_t i=0; i<num; i++)
    {
	  char *local =   geoip_query(finder, p[i]);
      printf("ip %s -> %s\n",p[i], local);
    }
    gettimeofday(&end,NULL);
    uint64_t us = (end.tv_sec - begin.tv_sec)*1000000 + (end.tv_usec - begin.tv_usec);
     printf("%lu us\n",us);
...
int main(int argc, char **argv)
{
    ...
    //test(finder);
	//char *ip = "92.65.134.68";
//	char *ip = "31.47.165.0";
//	char *ip = "58.69.179.87";
	//char *ip = "30.0.0.1";
	//char *ip = "1.0.101.0";
//	char *ip = "12.9.116.88";
	//char *ip = "154.0.0.222";
	char *ip = "24.103.50.232";
	char *local = geoip_query(finder, ip);
	printf("%s\n",local);

	ip = "197.20.100.150";
	local = geoip_query(finder, ip);
	printf("%s\n",local);
