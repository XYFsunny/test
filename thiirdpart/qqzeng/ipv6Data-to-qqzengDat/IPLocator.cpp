...
void IPSearch::printinfo()
{
    FILE*out = fopen("output", "w+");
    char buf[256] = {0};
    uint32_t my_index=0,startIp, endIp, local_offset, local_lengh;
    uint32_t first_index1 = first_index;
    uint32_t last_index1 = last_index;

    while(first_index1 < last_index1)
    {
        GetIndex(my_index, startIp, endIp, local_offset, local_lengh);
        //printf("num:%d, startip:%u, endip:%u, local_off:%u, local_len:%d\n",my_index, startIp, endIp, local_offset, local_lengh);
        sprintf(buf, "num:%d, startip:%u, endip:%u, local_off:%u, local_len:%d\n",my_index, startIp, endIp, local_offset, local_lengh);
